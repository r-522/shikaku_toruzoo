    ...
    <div class="card-body py-3">
      ...
      <div class="d-flex justify-content-between align-items-center mb-2">
        <div class="d-flex align-items-center gap-2 overflow-hidden">
          ...
          <FavoriteStar :is-favorite="user.is_favorite" @toggle="$emit('toggleFavorite', user)" class="flex-shrink-0" />
          <!-- ユーザー名: 詳細ページへのリンク（router-link でページ遷移） -->
          <router-link :to="`/community/${user.id}`" class="fw-bold text-decoration-none text-dark text-truncate">
            {{ user.username }}
          </router-link>
          <!-- グッドマーク: 1 つ以上合格がある場合に ✓ を表示 -->
          <GoodMark :has-good-mark="user.has_good_mark" class="flex-shrink-0" />
        </div>
        <!-- 総勉強時間（flex-shrink-0 で縮まないよう固定） -->
        <div class="small fw-bold flex-shrink-0 ms-2" style="color: #1A73E8">
          合計 {{ user.total_study_hours }}h
        </div>
      </div>

      <!-- 所持資格リスト（1 件以上ある場合のみ表示） -->
      <div v-if="user.certifications.length > 0" class="ps-2 ps-sm-4 mb-1">
        <div class="text-muted small fw-bold mb-1">所持資格</div>
        <!-- v-for: 配列の各要素を繰り返し表示する -->
        <div
          v-for="(cert, idx) in user.certifications"
          :key="'c' + idx"
          class="d-flex align-items-start gap-2 small flex-wrap"
          style="line-height: 1.7"
        >
          <!-- 「取得」バッジ（固定表示） -->
          <span class="badge flex-shrink-0" style="background-color: #2E7D32">取得</span>
          <span class="text-break">{{ cert.certification_name }}</span>
          <!-- 取得日（存在する場合のみ表示） -->
          <span v-if="cert.acquired_date" class="text-muted flex-shrink-0">({{ cert.acquired_date }})</span>
        </div>
      </div>

      <!-- 目標リスト（1 件以上ある場合のみ表示） -->
      <div v-if="user.goals.length > 0" class="ps-2 ps-sm-4">
        ...
        <div
          ...
          style="line-height: 1.7"
        >
          <!-- ステータスバッジ（受験日/合格/不合格/断念） -->
          <StatusBadge :status="goal.status" class="flex-shrink-0" />
          <span class="text-break">{{ goal.certification_name }}</span>
          <!-- 勉強時間（0 より大きい場合のみ表示） -->
          <span v-if="goal.study_hours > 0" class="text-muted flex-shrink-0">({{ goal.study_hours }}h)</span>
        </div>
      </div>
    </div>
  </div>
